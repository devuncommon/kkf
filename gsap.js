...
// GSAP SplitText Animations
	let addAnimation = function() {
		$(".no-split-text").css("opacity", 1);
		$(".rich-text-regular h5, .rich-text-regular h6, .rich-text-large h5, .rich-text-large h6").css("opacity", 1);

		$("h1:not(.no-split-text, .heading-hero), h2:not(.no-split-text), h3:not(.no-split-text), h4:not(.no-split-text)").each(function() {
			let t = $(this);
			let e = new SplitText(t[0], { type: "lines", linesClass: "word-line" });
			let i = $(e.lines);
		...
		$(".paragraph-s:not(.no-split-text), .paragraph-m:not(.no-split-text)").each(function() {
			let t = $(this);
			let e = new SplitText(t[0], { type: "lines", linesClass: "word-line" });
			let i = $(e.lines);
		...
		$(".rich-text-regular h5:not(.no-split-text), .rich-text-regular h6:not(.no-split-text), .rich-text-large h5:not(.no-split-text), .rich-text-large h6:not(.no-split-text)").each(function() {
			let t = $(this);
		...
		$("p:not(.no-split-text):not([class^='text-size-']):not(.text-size-18px), .text-size-18px:not(.no-split-text)").each(function() {
			let t = $(this);
